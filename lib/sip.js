...
Transport.prototype.send = function (msg, addr, port, name, cb) {
  ...
  // @todo - use stringifiers
  if (!readVia) {

    var via = 'SIP/' + msg.version + '/' + protocol.name.toUpperCase() + ' ';

    via += protocol.addr + ':' + protocol.port + ';branch=z9hG4bK' + new Date().getTime();
    via += ';rport';

    msg.setHeader('via', via, false);
  }
...
Transaction.prototype.__clientInvite = function (status) {
  ...
  // Send ACK request
  // ACK for non-200 responses must be not be send within transaction but sent directly to transport layer
  /*
  if (status >= 300) {
    var msgACK = msg.toRequest('ACK', null, true);
    var calleeContact = msg.headers.contact;

    if (calleeContact) {
      var uri = calleeContact.addr;

      if (calleeContact.params.rinstance) {
        uri += ';rinstance='+ calleeContact.params.rinstance;
      }

      msgACK.request.uri = uri;
      delete msgACK.headers.contact;
    }

    self.provider.send(msgACK);
  }
  */
 ...
 * @return {number} New state of transaction.
 */
Transaction.prototype.__serverNonInvite = function (status) {
...
Transaction.prototype._setState = function (state) {
  ...
  // transit to terminated state
  if (this._type == 0 && 
        ( this._isInvite && state == SIP_STATE_CONFIRMED ||
         !this._isInvite && state == SIP_STATE_COMPLETED ) ||
      this._type == 1 && state == SIP_STATE_COMPLETED) {
...
Transaction.prototype.send = function (msg, addr, port, name, cb) {
  ...
  // @temp - branch parameter is set in transport layer
  // @todo - move to TU
