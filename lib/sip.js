...
Message.prototype.getHeader = function (name, pos) {
  ...
  return isArray(header) ? header[pos] || null : header;
};
 ...
 * @param {boolean} append Push values to array.
 */
Message.prototype.setHeader = function (name, value, append) {

  if (!value) {
    return;
  }

  if (this.headers[name] && append) {
