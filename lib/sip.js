...
function parseParameters (value, sep, lower) {
  ...
  while (i <= len) {
    ...
    if (i == len && paramName || c == sep) {
      ...
      params[paramName] = lower ? value.substr(start, end).toLowerCase() : value.substr(start, end);
      start = i;
    }
    else if (c == EQUAL) {

      end = i - start - 1;
      paramName = value.substr(start, end).toLowerCase();
      start = i;
    } 
...
function parseVia (value) {
  ...
  return match ? {
    ...
    'params': port > 0 ? params && parseParameters(params) : port && parseParameters(por)
  } : null;
}
...
function parseUri (value, parse) {
  ...
  return {
    ...
    'headers': ( parse ? match[7] && parseParameters(match[7], '&') : match[7] ) || {}
  };
}
...
// ## Exports
//
// Exported functions - *parse*, *stringify*, *isMessage*,
// *createMessage* and *parseUri*.
var __parser;
